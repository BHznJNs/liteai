...
class TestArgumentsNormalizer:
    ...
    def test_arguments_normalizer_string_to_dict(self):
        ...
        result = _arguments_normalizer("name", '{"name": "Alice", "age": 30}')
    ...
    def test_arguments_normalizer_dict_passthrough(self):
        ...
        result = _arguments_normalizer("name", input_dict)
    ...
    def test_arguments_normalizer_empty_string(self):
        ...
        result = _arguments_normalizer("name", '{}')
        assert result == {}
    ...
    def test_arguments_normalizer_nested_structure(self):
        ...
        result = _arguments_normalizer("name", json_str)
        assert result == {
    ...
    def test_arguments_normalizer_chinese_characters_from_string(self):
        ...
        result = _arguments_normalizer("name", '{"message": "你好", "name": "张三"}')
        assert result == {"message": "你好", "name": "张三"}

        # JSON with Unicode escapes
        result = _arguments_normalizer("name", '{"message": "\\u4f60\\u597d"}')
    ...
    def test_arguments_normalizer_invalid_json(self):
        ...
        with pytest.raises(ToolArgumentParsingError):
            _arguments_normalizer("name", 'invalid json')

        with pytest.raises(ToolArgumentParsingError):
            _arguments_normalizer("name", '{incomplete')
...
class TestResultNormalizer:
    ...
    def test_result_normalizer_string_passthrough(self):
        ...
        assert _result_normalizer("name", "hello") == "hello"
        assert _result_normalizer("name", "") == ""
        assert _result_normalizer("name", "Hello, World!") == "Hello, World!"

    def test_result_normalizer_number_serialization(self):
        """Numbers should be converted to string representation"""
        assert _result_normalizer("name", 42) == "42"
        assert _result_normalizer("name", 3.14) == "3.14"
        assert _result_normalizer("name", 0) == "0"
        assert _result_normalizer("name", -100) == "-100"

    def test_result_normalizer_boolean_serialization(self):
        """Booleans should be converted to JSON boolean strings"""
        assert _result_normalizer("name", True) == "true"
        assert _result_normalizer("name", False) == "false"

    def test_result_normalizer_none_serialization(self):
        """None should be converted to JSON null"""
        assert _result_normalizer("name", None) == "null"

    def test_result_normalizer_dict_serialization(self):
        """Dictionaries should be serialized to JSON"""
        result = _result_normalizer("name", {"key": "value"})
        assert result == '{"key": "value"}'
        assert json.loads(result) == {"key": "value"}

        result = _result_normalizer("name", {"a": 1, "b": 2})
        parsed = json.loads(result)
        assert parsed == {"a": 1, "b": 2}

    def test_result_normalizer_list_serialization(self):
        """Lists should be serialized to JSON"""
        result = _result_normalizer("name", [1, 2, 3])
        assert result == "[1, 2, 3]"
        assert json.loads(result) == [1, 2, 3]

        result = _result_normalizer("name", ["a", "b", "c"])
    ...
    def test_result_normalizer_chinese_characters(self):
        ...
        # String passthrough
        assert _result_normalizer("name", "你好世界") == "你好世界"

        # Dict with Chinese
        result = _result_normalizer("name", {"message": "你好", "name": "张三"})
        assert "你好" in result
        assert "张三" in result
        parsed = json.loads(result)
    ...
    def test_result_normalizer_nested_structures(self):
        ...
        result = _result_normalizer("name", data)
        parsed = json.loads(result)
        assert parsed == data
    ...
    def test_result_normalizer_empty_collections(self):
        ...
        assert _result_normalizer("name", []) == []
        assert _result_normalizer("name", {}) == "{}"
        assert _result_normalizer("name", tuple()) == "[]"
